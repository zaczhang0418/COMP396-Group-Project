...
def save_equity_plot(oopnl: dict, pdres: dict, act: dict, outdir: Path):
    ...
    # Title with PD & activity & maxDD
    if pd_val is None:
        title = f"Portfolio Open→Open CumPnL | PD=NA | Activity={act_pct:.1f}% | MaxDD={maxdd:.2f}"
    else:
        title = f"Portfolio Open→Open CumPnL | PD={pd_val:.3f} | Activity={act_pct:.1f}% | MaxDD={maxdd:.2f}"
...
def _align_by_date(dates_a, vals_a, dates_b, vals_b):
    """
    Align two time series by date, returning (dates, a_vals, b_vals)
    for the intersection in chronological order.
    """
    import math
...
def _underwater_curve(cum):
    """
    Given a cumulative PnL series, return the underwater (drawdown) series.
    """
    runmax = []
    m = float("-inf")
    for v in cum:
...
def save_combined_equity_dashboard(oopnl: dict, realpnl: dict, pdres: dict, act: dict, outdir: Path):
    """
    Top: Portfolio CumPnL (Open→Open MTM vs Realized from fills)
    Bottom: Realized Underwater (drawdown)
    """
    # Extract
    ...
    pd_val = pdres.get("portfolio", {}).get("pd_ratio")
    act_pct = act.get("activity_pct", 0.0)
    title_pd = "NA" if pd_val is None else f"{pd_val:.3f}"
    ...
    ax1.set_title(f"Portfolio CumPnL — MTM vs Realized | PD={title_pd} | Activity={act_pct:.1f}%")
...
def save_per_series_pd(pdres: dict, outdir: Path):
    rows = []
    for name, d in pdres.get("per_instrument", {}).items():
        rows.append({
            "series": name,
            "pd_ratio": d.get("pd_ratio"),
...
# --- NEW HELPERS (append to framework/plotting.py) ---------------------------
...
def _underwater(cum):
    """Drawdown series (running max - cum)."""
    rm = _running_max(cum)
...
def _max_dd_cycle_indices(cum):
    """
    Return (peak_idx, trough_idx, recovery_idx) for the *max depth* drawdown cycle,
    where recovery_idx is first index >= trough where cum >= cum[peak].
    If never recovers, recovery_idx = len(cum)-1.
    """
    if not cum:
...
def save_equity_dashboard(oopnl: dict, pdres: dict, act: dict, outdir: Path):
    """
    Big dashboard:
      - Top row: portfolio equity with max DD shaded and *duration* highlighted.
      - Bottom: 2 cols x 5 rows: each instrument equity curve.
    """
    dates = oopnl.get("dates", [])
    eq = oopnl.get("portfolio_cum", [])
    ...
    gs = gridspec.GridSpec(nrows=6, ncols=2, height_ratios=[2, 1, 1, 1, 1, 1], hspace=0.45, wspace=0.15)
    ...
    # Break-even
    ax_top.axhline(0, linestyle="--", linewidth=1, alpha=0.7, label="Break-even")
    ...
    pd_val = pdres.get("portfolio", {}).get("pd_ratio")
    act_pct = act.get("activity_pct", 0.0)
    # Max DD depth for title
    uw = _underwater(eq) if eq else []
    maxdd_depth = max(uw) if uw else 0.0
    title = f"Portfolio Open→Open CumPnL | PD={pd_val:.3f} | Activity={act_pct:.1f}% | MaxDD={maxdd_depth:.2f}" if pd_val is not None \
        else f"Portfolio Open→Open CumPnL | PD=NA | Activity={act_pct:.1f}% | MaxDD={maxdd_depth:.2f}"
    ...
    for k, name in enumerate(names_sorted):
        ...
        c = (k % cols)
        ax = fig.add_subplot(gs[r, c], sharex=ax_top)
        ax.plot(dates, per[name], linewidth=1.2)
        ax.set_title(name)
...
def save_all_series_equity(oopnl: dict, outdir: Path):
    """
    Plot all per-instrument equity curves on a single axes.
    """
    dates = oopnl.get("dates", [])
    per = oopnl.get("per_instrument_cum", {})
    if not per:
...
def save_portfolio_underwater(oopnl: dict, outdir: Path):
    """
    Underwater chart: drawdown vs running peak for the portfolio.
    """
    dates = oopnl.get("dates", [])
    eq = oopnl.get("portfolio_cum", [])
