# -*- coding: utf-8 -*-
#!/usr/bin/env python3
# Scripts/coursework_2/stage_2_single_strategy_optimization/mr/run_grid_search.py
import argparse
import itertools
import json
import subprocess
...
STRATEGY = "coursework_2.stage_2_mr_generic_v2"
DATA_NAME = "series_10"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--asset-tag", default="asset10")
    ...
    for i, combo in enumerate(combos, 1):
        ...
        print(f"[{i}/{total}] START -> {run_dir} params={search_params}", flush=True)
        ...
        try:
            ...
            print(f"[{i}/{total}] FAIL -> {run_dir} ({exc})", flush=True)
            metrics = {
                "true_pd_ratio": 0.0,
        ...
        print(f"[{i}/{total}] OK -> {run_dir}", flush=True)
