# -*- coding: utf-8 -*-
#!/usr/bin/env python3
# Scripts/coursework_2/stage_2_single_strategy_optimization/tf/pick_best.py
import argparse
import json
import subprocess
import sys
...
RUN_ONCE = PROJ / "Scripts" / "coursework_2" / "stage_2_single_strategy_optimization" / "tf" / "run_once.py"
DEFAULT_EXPERIMENT_TAG = "adhoc"
DEFAULT_GRID_CONFIG = PROJ / "Configs" / "grids_for_single_strategies" / "tf" / "refined" / "refined_v1.json"
...
if __name__ == "__main__":
    ...
    out_root = get_coursework_2_stage2_dir(args.experiment_tag, "part1", "tf", "grid_search")
    results = out_root / "results.csv"
    best_out = out_root / "best_params.json"
    best_runs_root = get_coursework_2_stage2_dir(args.experiment_tag, "part1", "tf", "best_runs")
    ...
    if args.key not in header:
        raise SystemExit(f"Key {args.key} not in header")
