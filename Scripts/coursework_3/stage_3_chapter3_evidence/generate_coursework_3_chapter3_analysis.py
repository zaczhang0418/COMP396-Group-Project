...
OUT_DIR = ROOT / "Output" / "coursework_3" / "stage_2_team01_v3_final" / "chapter3_analysis"
SOURCE_REF = "final/submission-integration"

PATHS = {
    "v1_matrix_summary": "Output/coursework_1/stage_5_team01_v1_archive/matrix_summary.csv",
    "v1_part3_per_series": "Output/coursework_1/stage_5_team01_v1_archive/PART3/combo_tf01_mr10_garch07_v1/per_series_pd.json",
    "v2_process_summary": "Output/coursework_2/stage_4_team01_v2_archive/process_summary.csv",
    "v2_per_leg_summary": "Output/coursework_2/stage_4_team01_v2_archive/per_leg_summary.csv",
    "v3_process_summary": "Output/coursework_3/stage_2_team01_v3_final/process_summary.csv",
    "v3_part1_per_series": "Output/coursework_3/stage_2_team01_v3_final/team01_v3_final/part1/per_series_pd.json",
    "v3_part2_per_series": "Output/coursework_3/stage_2_team01_v3_final/team01_v3_final/part2/per_series_pd.json",
    "v3_part3_per_series": "Output/coursework_3/stage_2_team01_v3_final/team01_v3_final/part3/per_series_pd.json",
    "v3_cross_asset_long": "Output/coursework_3/stage_1_cross_asset_scan/summaries/cross_asset_long.csv",
}

OLD_TO_NEW_OUTPUT_PATHS = {
    ...
    "Output/coursework_3_stage3_v2v3_analysis": "Output/coursework_3/stage_3_chapter3_evidence",
}
...
def normalize_source_path(path: str) -> str:
    ...
    for old, new in OLD_TO_NEW_OUTPUT_PATHS.items():
...
def read_text(repo_path: str) -> str:
    return (ROOT / repo_path).read_text(encoding="utf-8-sig").lstrip("\ufeff")


def read_csv(repo_path: str) -> list[dict[str, str]]:
    text = read_text(repo_path)
    return list(csv.DictReader(io.StringIO(text)))


def read_json(repo_path: str):
    return json.loads(read_text(repo_path))
...
def load_v1_progression_rows() -> list[dict]:
    rows = []
    matrix_rows = read_csv(PATHS["v1_matrix_summary"])
...
def load_v2_progression_rows() -> list[dict]:
    rows = []
    process_rows = read_csv(PATHS["v2_process_summary"])
        ...
        rows.append(
            {
                ...
                "bankrupt": to_bool(row["bankrupt"]),
                "source_ref": SOURCE_REF,
                "source_path": normalize_source_path(row["source_path"]),
            }
        )
    return rows
...
def load_v3_progression_rows() -> list[dict]:
    rows = []
    process_rows = read_csv(PATHS["v3_process_summary"])
    for row in process_rows:
        rows.append(
            {
                "version": "V3",
                ...
                "bankrupt": to_bool(row["bankrupt"]),
                "source_ref": SOURCE_REF,
                "source_path": normalize_source_path(row["output_path"]),
            }
        )
    return rows
...
def load_part3_leg_rows() -> list[dict]:
    ...
    v1_payload = read_json(PATHS["v1_part3_per_series"])
    rows.extend(
        normalize_leg_rows(
            version="V1",
            dataset_label="PART3",
    ...
    v2_rows = read_csv(PATHS["v2_per_leg_summary"])
    v2_payload = []
    for row in v2_rows:
    ...
    v3_payload = read_json(PATHS["v3_part3_per_series"])
    rows.extend(
        normalize_leg_rows(
            version="V3",
            dataset_label="part3",
...
def build_asset_reassignment_evidence() -> list[dict]:
    cross_asset_rows = read_csv(PATHS["v3_cross_asset_long"])
    ...
    return [
        {
            ...
            "source_ref": SOURCE_REF,
            "source_path": "Strategies/coursework_1/stage_4_initial_combo_team01_v1.py",
        },
        {
            "version": "V2",
            "version_role": "submitted baseline",
            ...
            "source_ref": SOURCE_REF,
            "source_path": "Strategies/coursework_2/stage_3_team01_v2_presubmission.py",
        },
        {
            "version": "V3",
            "version_role": "CA3 refinement",
            ...
            "source_ref": SOURCE_REF,
            "source_path": "Strategies/coursework_3/stage_2_team01_v3_final.py",
        },
    ]
