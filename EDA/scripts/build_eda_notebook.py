...
NOTEBOOK_PATH = ROOT / "EDA" / "notebooks" / "EDA_Report_and_Justification.ipynb"
OVERVIEW_DIR = ROOT / "EDA" / "output" / "stage4_overview"
ANALYSIS_REPORT = ROOT / "EDA" / "docs" / "latest-analysis.md"
...
def _dataset_overview_text() -> str:
    path = OVERVIEW_DIR / "dataset_summary.csv"
    if not path.exists():
        return "dataset_summary.csv is missing. Run .\\EDA\\run_all_eda.bat OVERVIEW first."
...
def _asset_snapshot_text(dataset: str = "PART123") -> str:
    path = OVERVIEW_DIR / "asset_summary.csv"
    if not path.exists():
        return "asset_summary.csv is missing. Run .\\EDA\\run_all_eda.bat OVERVIEW first."
...
def _part123_analysis_text() -> str:
    if not ANALYSIS_REPORT.exists():
        return "latest-analysis.md is missing. Run .\\EDA\\run_all_eda.bat ALL first."

    text = ANALYSIS_REPORT.read_text(encoding="utf-8", errors="replace")
    start = text.find("## PART123")
    if start == -1:
        return "PART123 section was not found in latest-analysis.md."
...
def build_notebook() -> None:
    ...
    chart_paths = [
        OVERVIEW_DIR / "total_return_heatmap.png",
        OVERVIEW_DIR / "risk_return_overview.png",
        OVERVIEW_DIR / "row_coverage.png",
    ]
...
""",
        ...
        _cell(
            "code",
            """dataset_summary = pd.read_csv(ROOT / "EDA/output/stage4_overview/dataset_summary.csv")
dataset_summary""",
...
for chart in [
    ROOT / "EDA/output/stage4_overview/total_return_heatmap.png",
    ROOT / "EDA/output/stage4_overview/risk_return_overview.png",
    ROOT / "EDA/output/stage4_overview/row_coverage.png",
]:
...
The following values are extracted from `EDA/docs/latest-analysis.md`, which is generated from the EDA run logs.
""",
        ),
        _cell(
            "code",
            """analysis_text = (ROOT / "EDA/docs/latest-analysis.md").read_text(encoding="utf-8")
...
""",
        ...
        _cell(
            "code",
            """asset_summary = pd.read_csv(ROOT / "EDA/output/stage4_overview/asset_summary.csv")
...
The full analysis text is saved to `EDA/docs/latest-analysis.md`; run status is saved to `EDA/docs/latest-run.md`.
""",
        ),
    ]
